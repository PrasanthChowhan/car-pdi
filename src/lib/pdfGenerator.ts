...
export async function generatePDIReport(
  ...
  doc.setFontSize(10);
  doc.setTextColor(cInk[0], cInk[1], cInk[2]);
  doc.text('ACKNOWLEDGEMENTS & SIGN-OFF', margin, currentY);
  currentY += 4;

  doc.setFont('Helvetica', 'normal');
  doc.setFontSize(8.5);
  doc.setTextColor(cBody[0], cBody[1], cBody[2]);
  doc.text('By signing below, both parties acknowledge the correctness of the findings logged in this report.', margin, currentY);
  currentY += 8;

  // Customer Signature Box
  doc.setDrawColor(cHairline[0], cHairline[1], cHairline[2]);
  doc.rect(margin, currentY, 82, 38, 'D');
  doc.setFont('Helvetica', 'bold');
  doc.setFontSize(9);
  doc.setTextColor(cInk[0], cInk[1], cInk[2]);
  ...
  doc.rect(margin + 88, currentY, 92, 38, 'D');
  doc.setFont('Helvetica', 'bold');
  doc.setFontSize(9);
  doc.setTextColor(cInk[0], cInk[1], cInk[2]);
  ...
  doc.setDrawColor(cHairline[0], cHairline[1], cHairline[2]);
  doc.rect(margin + 156, currentY + 10, 20, 20, 'D');
  doc.setFontSize(7);
  doc.setTextColor(cMuted[0], cMuted[1], cMuted[2]);
